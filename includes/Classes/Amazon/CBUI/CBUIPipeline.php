 ...
 */
require_once('Classes/Amazon/.config.inc.php');
$debug->newFile('CBUIPipeline.php');
abstract class Amazon_CBUI_CBUIPipeline {
	...
	*/
	protected static $CBUI_URL = AMAZON_CBUI_URL;//Turn testing on or off in .config.inc.php.
	...
	*/
	function Amazon_CBUI_CBUIPipeline($pipelineName, $awsAccessKey, $awsSecretKey){
	...
	*/
	private static function _calculateStringToSignV2(array $parameters, $httpMethod, $hostHeader, $requestURI) {
		...
		$uriencoded = implode("/", array_map(array("Amazon_CBUI_CBUIPipeline", "_urlencode"), explode("/", $requestURI)));
		$data .= $uriencoded;
		$data .= "\n";
