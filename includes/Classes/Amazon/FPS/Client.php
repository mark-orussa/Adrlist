<?php $fileInfo = array('title' => 'Payment Authorization', 'fileName' => 'Amazon/FPS/Client.php');
...
class Amazon_FPS_Client implements Amazon_FPS_Interface
{
    ...
    /** @var array */
	//Turn testing on or off in .config.inc.php.
    private  $_config = array('ServiceURL' => AMAZON_API_URL,
        ...
        try{
            ...
            do{
                ...
                //Rethrow on deserializer error.
					...
					//throw new Adrlist_CustomException('','ResponseBody: ' . $response['ResponseBody'] . 'Status: ' . $response['Status']);
    ...
    private function _reportAnyErrors($responseBody, $status, Exception $e =  null){
        ...
        if(!is_null($responseBody) && strpos($responseBody, '<') === 0){
            if(preg_match('@<RequestId>(.*)</RequestId>.*<Error><Code>(.*)</Code><Message>(.*)</Message></Error>.*(<Error>)?@mi', $responseBody, $errorMatcherOne)){
                ...
                //require_once('Amazon/FPS/Exception.php');
                $ex = new Amazon_FPS_Exception(array(
					'Message' => $message,
					'StatusCode' => $status,
					'ErrorCode' => $code,
					'ErrorType' => 'Unknown',
    ...
    private function _httpPost(array $parameters){
        ...
        //Grab the response code only from the header.
        ...
        curl_close($ch);
		$curlReturn = array('Status' => (int)$code, 'ResponseBody' => $responseBody);
		$debug->printArray($curlReturn,'$curlReturn');
        return $curlReturn;
    }
    ...
    private function _pauseOnRetry($retries, $status){
        if($retries <= $this->_config['MaxErrorRetry']){
            ...
            usleep($delay);//Delay execution in microseconds.
        }else{
            //require_once('Amazon/FPS/Exception.php');
